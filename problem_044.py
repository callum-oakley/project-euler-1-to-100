...
def pentagonalDifferences():
    n, pentagonals, maxN, minDifference = 2, set(), 0, float("inf")
    ...
    while p(n) - p(n - 1) < minDifference:
        m = n - 1
        while m > 0 and p(n) - p(m) < minDifference:
            while p(maxN) < p(n) + p(m):
                maxN += 1
                pentagonals.add(p(maxN))
            if p(n) - p(m) in pentagonals and p(n) + p(m) in pentagonals:
                minDifference = p(n) - p(m)
                yield minDifference
            m -= 1
        n += 1

print(min(pentagonalDifferences()))
